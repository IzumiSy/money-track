 ...
 */
function createMonthlyContext(
  overrides: Partial<MonthlyProcessingContext> & {
  ...
  return {
    monthIndex: 0,
    assetBalances: new Map<string, number>(),
    liabilityBalances: new Map<string, number>(),
    incomeBreakdown: new Map<string, number>(),
    expenseBreakdown: new Map<string, number>(),
    allSources: [],
    ...overrides,
  };
}
 ...
 */
function createPostMonthlyContext(
  overrides: Partial<PostMonthlyContext> & {
...
describe("AssetPlugin", () => {
  describe("applyMonthlyEffect", () => {
    it("積立（expense）がある場合、残高が増加する", () => {
      ...
      expect(assetBalances.get("asset-1")).toBe(1050000);
      expect(expenseBreakdown.get("investment_asset-1")).toBe(50000);
    });
    ...
    it("引き出し（income）がある場合、残高が減少する", () => {
      ...
      expect(assetBalances.get("asset-1")).toBe(900000);
    });
    ...
    it("積立と引き出しが両方ある場合、正味の変動が適用される", () => {
      ...
      expect(assetBalances.get("asset-1")).toBe(1030000);
      expect(expenseBreakdown.get("investment_asset-1")).toBe(50000);
    });
    ...
    it("残高が0から始まる場合でも正しく処理される", () => {
      ...
      expect(assetBalances.get("asset-1")).toBe(30000);
    });
  });
  ...
  describe("postMonthlyProcess - 利回り計算", () => {
    it("年利5%の資産に対して月次利息が計算される", () => {
      ...
      expect(assetBalances.get("asset-1")).toBe(1200000 + expectedInterest);
      expect(incomeBreakdown.get("return_asset-1")).toBe(expectedInterest);
    });
    ...
    it("利回りが0%の場合、利息は発生しない", () => {
      ...
      expect(assetBalances.get("asset-1")).toBe(1000000);
      expect(incomeBreakdown.has("return_asset-1")).toBe(false);
    });
    ...
    it("複数の資産に対して個別に利息が計算される", () => {
      ...
      expect(assetBalances.get("asset-1")).toBe(1000000 + expectedInterest1);
      expect(assetBalances.get("asset-2")).toBe(500000 + expectedInterest2);
      expect(incomeBreakdown.get("return_asset-1")).toBe(expectedInterest1);
      expect(incomeBreakdown.get("return_asset-2")).toBe(expectedInterest2);
    });
    ...
    it("利息が複利で積み上がることを確認", () => {
      ...
      // 1000000 * 0.01 = 10000
      expect(assetBalances.get("asset-1")).toBe(1010000);
      ...
      // 1010000 * 0.01 = 10100
      expect(assetBalances.get("asset-1")).toBe(1020100);

      // 利息の合計: 10000 + 10100 = 20100
      expect(incomeBreakdown.get("return_asset-1")).toBe(20100);
    });
    ...
    it("非資産タイプのソースは利息計算されない", () => {
      ...
      expect(assetBalances.get("asset-1")).toBe(1000000 + expectedInterest);
      expect(incomeBreakdown.get("return_asset-1")).toBe(expectedInterest);

      // 収入ソースには影響なし
      expect(incomeBreakdown.has("return_income_income-1")).toBe(false);
    });
  });
